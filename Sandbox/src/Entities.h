...
void CreateCube()
{
	...
	Wiwa::Mesh mesh;
	mesh.meshId = Wiwa::Resources::Load<Wiwa::Model>("resources/meshes/cube.fbx");
...
void CreatePlane()
{
	EntityId myEntity = Wiwa::Application::Get().GetEntityManager().CreateEntity("resources/meshes/plane.fbx");
	Wiwa::Application::Get().GetEntityManager().AddComponent<Wiwa::Transform3D>(myEntity, { {0,0,0},{0.0f, 0.0f, 0.0f},{1.0,1.0, 1.0} });
	Wiwa::Mesh mesh;
